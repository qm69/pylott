...
def draw_date(game):
    ...
    florida_now = datetime.now(florida)
    ...
    year, day, month, weekday = date_tupl[0], date_tupl[1], date_tupl[2], date_tupl[6]

    if game == 'cash_3' or game == 'play_4':
        # Midday @ 13:30 [17:57 GMT, +1 day 20:30 Kiev]
        # Evening @ 19:57 [23:57 GMT, +1 day 02:57 Kiev]
        cash3_draw_time = datetime(year, day, month, 13, 30, 0, 0, florida)
        if florida_now > cash3_draw_time:
            return florida_now.date()
        else:
            return (florida_now - timedelta(days=1)).date()
        ...
        if weekday in [1, 4] and florida_now.time() > time(23, 15, 0):
            ...
            return (florida_now - timedelta(days=minus_day)).date()
    else:
        raise Exception('No such game like: {}'.format(game))

if __name__ == '__main__':
    print(draw_date('play_4'))
    print(draw_date('lucky_money'))
